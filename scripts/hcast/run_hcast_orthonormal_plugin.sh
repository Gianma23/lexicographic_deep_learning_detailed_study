...
# Runs H-CAST with a frozen orthonormal taxonomy frame directly after the
# concatenated level logits. Lexicographic gradient projection is disabled.
# Defaults:
# - orthonormal_plugin.loss=global_softmax_ce_reg
# - orthonormal_plugin.weight_mode=equal
# - orthonormal_plugin.transform_mode=final_only
...
run_output_dir() {
  ...
  echo "$OUTPUTS_ROOT/hcast_${ds}_orthonormal_plugin_${LOSS_MODE}_${PLUGIN_TRANSFORM_MODE}${frame_suffix}"
}
...
printf 'Completed all requested H-CAST final-only non-lex plugin runs.\n'
