...
class HTCapsNet(nn.Module):
    """PyTorch port of the released TensorFlow HT-CapsNet architecture."""
    ...
    def __init__(
        ...
        if not isinstance(backbone_preprocessing, str) or backbone_preprocessing not in {
            ...
            raise ValueError(
                "HT-CapsNet backbone_preprocessing must be keras or timm."
            )
    ...
    def _build_primary_caps(self, feat: torch.Tensor) -> torch.Tensor:
        ...
        if flat.size(1) % self.primary_dim != 0:
            raise RuntimeError(
                ...
                f"capsule dim ({self.primary_dim})."
            )
        caps = flat.view(
            bsz,
            self.primary_caps_count,
            self.primary_dim,
        )
    ...
    def forward(self, x: torch.Tensor, targets: Optional[torch.Tensor] = None) -> Dict[str, Any]:
        ...
        for level in range(self.depth):
            if level == 0:
                ...
                # The released layer reinterprets its one already-squashed 8-D
                # primary tensor at each preceding secondary-capsule dimension.
                p_caps_lvl = self._reshape_primary_for_level(
                    primary_caps,
                    self.secondary_dims[level - 1],
                )
