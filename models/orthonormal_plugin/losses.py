...
from .config import get_section, plugin_section, section_to_dict
...
def _hard_targets_from_input(targets: PluginTargets, num_levels: int) -> torch.Tensor:
    ...
    if not isinstance(hard_targets, torch.Tensor):
        raise TypeError("Orthonormal plugin expects hard targets as tensor [B, L].")
    if hard_targets.ndim != 2 or int(hard_targets.size(1)) != int(num_levels):
        raise ValueError(
            f"Orthonormal plugin expected hard targets with shape [B, {num_levels}], "
...
def _loss_settings(cfg: Any) -> Tuple[Dict[str, Any], str, str, float]:
    ...
    if bool(plugin_cfg.get("enabled", False)):
        return plugin_cfg, "orthonormal_plugin", "global_softmax_ce_reg", 0.05
...
def _validate_plugin_output(
    ...
    node_logits = _get_tensor(output, "orthonormal_plugin_node_logits", "node_logits")
    if not isinstance(node_logits, torch.Tensor) or node_logits.ndim != 2:
        raise ValueError("Orthonormal plugin output must provide `node_logits` with shape [B, N].")

    level_node_ids = _get_list(
        output,
        "orthonormal_plugin_level_node_ids",
        "hiercos_level_node_ids",
    )
    if not isinstance(level_node_ids, list) or len(level_node_ids) != num_levels:
        raise ValueError("Orthonormal plugin output must provide level node ids aligned with hierarchy depth.")
    if not all(isinstance(node_ids, torch.Tensor) and node_ids.ndim == 1 for node_ids in level_node_ids):
        raise ValueError("Orthonormal plugin level node id entries must be 1D tensors.")

    leaf_to_level_local = _get_tensor(
        output,
        "orthonormal_plugin_leaf_to_level_local",
        "leaf_to_level_local",
    )
    if not isinstance(leaf_to_level_local, torch.Tensor) or leaf_to_level_local.ndim != 2:
        raise ValueError("Orthonormal plugin output must provide `leaf_to_level_local` with shape [num_leaf, depth].")
    if int(leaf_to_level_local.size(1)) != num_levels:
        raise ValueError(
            "Orthonormal plugin `leaf_to_level_local` depth mismatch: "
            f"expected {num_levels}, found {int(leaf_to_level_local.size(1))}."
        )

    node_prob_weights = _get_tensor(
        output,
        "orthonormal_plugin_node_prob_weights",
        "node_prob_weights",
    )
    if not isinstance(node_prob_weights, torch.Tensor) or node_prob_weights.ndim != 1:
        raise ValueError("Orthonormal plugin output must provide `node_prob_weights` with shape [depth].")
    if int(node_prob_weights.numel()) != num_levels:
        raise ValueError(
            "Orthonormal plugin `node_prob_weights` depth mismatch: "
...
def _path_global_node_ids(
    ...
    for level, level_nodes in enumerate(level_node_ids):
        ...
        _check_index_range(local_ids, level_size, f"Orthonormal plugin invalid local node indices for level {level}.")
...
def _weighted_target_ce_level_losses(
    ...
    if softmax_scope not in {"global", "level"}:
        raise ValueError(f"Unsupported Orthonormal plugin softmax scope '{softmax_scope}'.")
    ...
    for level, level_nodes in enumerate(level_node_ids):
        ...
        level_size = int(level_nodes.numel())
        if level_size <= 0:
            raise ValueError(f"Orthonormal plugin level {level} has no node ids.")
...
def _shared_level_weights(
    ...
    if not isinstance(node_prob_weights, torch.Tensor) or node_prob_weights.ndim != 1:
        raise ValueError(
            "Orthonormal plugin weight modes 'kl_leaf' and 'kl_coarse' require "
            "`node_prob_weights` with shape [depth]."
        )
    if int(node_prob_weights.numel()) != int(num_levels):
        raise ValueError(
            "Orthonormal plugin `node_prob_weights` depth mismatch for level weighting: "
            f"expected {num_levels}, found {int(node_prob_weights.numel())}."
        )

    weights = node_prob_weights.to(device=device, dtype=dtype)
    if bool((weights < 0).any()):
        raise ValueError("Orthonormal plugin weighting requires non-negative `node_prob_weights`.")
...
def compute_loss(
    ...
    if isinstance(targets, dict) and targets.get("soft_targets_per_level") is not None:
        raise ValueError("Orthonormal plugin does not support mixup/cutmix soft targets. Disable mixup/cutmix.")

    logits_per_level = output.get("logits_per_level")
    if not isinstance(logits_per_level, list) or not logits_per_level:
        raise ValueError("Orthonormal plugin output must contain non-empty `logits_per_level`.")
    ...
    _check_index_range(
        leaf_targets,
        num_leaf,
        f"Orthonormal plugin leaf targets out of range [0, {num_leaf}). "
