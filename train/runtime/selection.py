...
def selection_key(eval_metrics: Mapping[str, float], mode: str) -> SelectionKey:
    ...
    Every model family shares one rule: the lexicographic order FPA (higher),
    TICE (lower), then weighted AP (higher).

    Missing or non-finite values rank below every finite value. Deepest-level
    accuracy is the fallback when none of the three hierarchy metrics is
    present.
    """
